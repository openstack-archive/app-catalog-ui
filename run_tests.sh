...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run App Catalog Horizon's test suite(s)"
  echo ""
...
function run_pep8 {
  ...
  DJANGO_SETTINGS_MODULE=app_catalog.tests.settings ${command_wrapper} flake8
}
...
function run_pep8_changed {
    ...
    warn_on_flake8_without_venv
    diff -u --from-file /dev/null ${files} | DJANGO_SETTINGS_MODULE=app_catalog.tests.settings ${command_wrapper} flake8 --diff
    exit
}

function run_sphinx {
    echo "Building sphinx..."
    DJANGO_SETTINGS_MODULE=app_catalog.tests.settings ${command_wrapper} python setup.py build_sphinx
    echo "Build complete."
}
...
function run_tests_all {
  echo "Running app_catalog application tests"
  export NOSE_XUNIT_FILE=app_catalog/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='app_catalog_nose_results.html'
  fi
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=app_catalog/tests/integration_tests"
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test app_catalog --settings=app_catalog.tests.settings $testopts
  # get results of the Horizon tests
  APP_CATALOG_DASHBOARD_RESULT=$?
  ...
  TEST_RESULT=$(($APP_CATALOG_DASHBOARD_RESULT || $PEP8_RESULT))
...
function run_integration_tests {
  ...
  export HORIZON_INTEGRATION_TESTS_CONFIG_FILE="app_catalog/tests/integration_tests/horizon.conf"

  echo "Running App Catalog Horizon integration tests..."
  if [ -z "$testargs" ]; then
      ${command_wrapper} nosetests app_catalog/tests/integration_tests/tests
  else
...
function run_makemessages {
  OPTS="-l en --no-obsolete --settings=openstack_dashboard.tests.settings"
...
function run_compilemessages {
  OPTS="--settings=openstack_dashboard.tests.settings"
  cd horizon
